...
public class OperateOracle {
    ...
    public static Connection getConnection(String urlx, String usernamex, String passwordx) {
        try {
            Class.forName(DRVIER);
            // TODO 设置连接超时
            DriverManager.setLoginTimeout(30);
            connection = DriverManager.getConnection(urlx, usernamex, passwordx);
//            connection.setNetworkTimeout(Executors.newSingleThreadExecutor(), 1000 * 30);
            System.out.println("成功连接数据库");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("class not find !", e);
        } catch (SQLException e) {
